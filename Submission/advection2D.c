...
Outputs: initial.dat - inital values of u(x,y)
		...
		Serial compile: 	gcc -o advection2D -std=c99 advection2D.c -lm

		Parallel Compile: 	gcc -fopenmp -o advection2D -std=c99 advection2D.c -lm
...
#pragma omp parallel for collapse(2) default(none) shared(velx, y, z0, ustar, k, NX, NY)
...
#pragma omp parallel for collapse(2) default(none) shared(u, x, y, x0, y0, sigmax2, sigmay2, NY, NX) private(x2, y2)
	...
	/* Cannot be parallalised as contents of the initial file must be in sequential order */
	/* LOOP 4 */
	for (int i = 0; i < NX + 2; i++)
	{
		for (int j = 0; j < NY + 2; j++)
...
#pragma omp parallel for collapse(2) default(none) shared(u, dx, dy, NX, NY, velx, vely, dudt)
		for (int i = 1; i < NX + 1; i++)
		{
			...
			{
				dudt[i][j] = -velx[i][j] * (u[i][j] - u[i - 1][j]) / dx - vely * (u[i][j] - u[i][j - 1]) / dy;
			}
		}
...
#pragma omp parallel for collapse(2) default(none) shared(u, dudt, dt, NX, NY)
	...
	/* cannot be parallalised as file content must be in sequential order */
	/* LOOP 10 */
	for (int i = 0; i < NX + 2; i++)
	{
		for (int j = 0; j < NY + 2; j++)
